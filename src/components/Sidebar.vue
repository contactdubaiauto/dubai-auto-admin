<template>
	<div class="p-3 h-full w-full border-r border-gray-200 flex flex-col">
			...
			<PanelMenu :model="items" class="w-full" multiple>
				<template #item="{ item }">
					<router-link v-if="item.route" v-slot="{ href, navigate }" :to="item.route" custom>
						<a class="flex items-center cursor-pointer text-gray-700 dark:text-gray-0 px-4 py-2 rounded-md" :href="href"
							@click="navigate">
							<span :class="item.icon" />
							<span class="ml-2">{{ item.label }}</span>
							<span v-if="item.route === '/chat' && totalUnreadCount > 0"
								class="ml-auto px-2 py-0.5 bg-blue-600 text-white text-xs font-semibold rounded-full min-w-[1.25rem] text-center">
								{{ totalUnreadCount > 99 ? '99+' : totalUnreadCount }}
							</span>
						</a>
					</router-link>
					<a v-else class="flex items-center cursor-pointer text-gray-700 dark:text-gray-0 px-4 py-2 rounded-md"
		...
		<div class="pt-2 w-full">
			<SelectButton v-model="selectedLanguage" @change="selectLanguage" option-label="title" option-value="code"
				:options="languages" fluid />
		</div>
		<div class="pt-2">
		...
		},
		{
			...
			items: [
				...
				},
				{
					label: t('sidebar.brands'),
					icon: 'pi pi-database',
					route: '/motorcycle-settings/brands'
				},
				{
					label: t('sidebar.engines'),
					icon: 'pi pi-database',
					route: '/motorcycle-settings/engines'
				},
				{
					label: t('sidebar.strokes'),
					icon: 'pi pi-database',
					route: '/motorcycle-settings/strokes'
				}
			]
		},
		{
