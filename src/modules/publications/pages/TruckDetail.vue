	...
	<div class="h-full flex flex-col">
		<div class="flex flex-col gap-4 p-4 border-b-2 border-gray-100 mb-2">
			<div class="flex justify-between w-full items-center">
				<Button @click="goBack" icon="pi pi-arrow-left" :label="t('publications.truckDetail.back')"
					severity="secondary" />
				<div class="flex items-center gap-2">
					<div v-if="truck" class="text-sm text-gray-700 font-semibold">
						<span :class="[
							'inline-flex items-center px-2 py-0.5 rounded text-xs font-medium',
							truck.moderation_status === 1 ? 'bg-yellow-100 text-yellow-800' :
								truck.moderation_status === 2 ? 'bg-green-100 text-green-800' :
									truck.moderation_status === 3 ? 'bg-red-100 text-red-800' :
										'bg-gray-100 text-gray-800'
						]">
							{{ getStatusLabel(truck.moderation_status) }}
						</span>
					</div>
		...
		<div class="flex-1 overflow-y-auto p-4">
			...
			<div v-else-if="truck" class="max-w-4xl mx-auto">
				<div class="mb-6">
					<div class="grid grid-cols-2 md:grid-cols-4 gap-4">
						<template v-if="truck.images && truck.images.length > 0">
							<div v-for="(image, index) in truck.images.slice(0, 4)" :key="index" class="relative"
								style="aspect-ratio: 4 / 3; background-color: #f3f4f6; border-radius: 0.5rem; overflow: hidden;">
								<img :src="`${image}_l.jpg`" :alt="`${truck.comtran_brand} ${truck.comtran_model} - ${index + 1}`"
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						...
						<div class="flex items-center gap-2 mb-4">
							<div v-if="truck.owner.avatar" class="w-8 h-8 rounded-full overflow-hidden">
								<img :src="`${truck.owner.avatar}_l.jpg`" :alt="truck.owner.username || ''"
									class="w-full h-full object-cover" />
							</div>
							<div v-else class="w-8	 h-8 rounded-full bg-gray-200 flex items-center justify-center">
								<span class="text-l text-gray-600">{{ (truck.owner.username || 'U').slice(0, 1).toUpperCase() }}</span>
							</div>
							<div>
								<div v-if="truck.owner.username" class="font-semibold text-gray-800">{{ truck.owner.username }}
								</div>
							</div>
						</div>
					</div>
					<div v-if="truck.owner.role_id">
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.userRole') }}</div>
						<div class="font-semibold text-gray-800">{{ getRoleLabel(truck.owner.role_id) }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.city') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.city }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.status') }}</div>
						<div class="font-semibold text-gray-800">{{ getStatusLabel(Number(truck.status)) }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.price') }}</div>
						<div class="font-semibold text-gray-800">{{ formatPrice(truck.price) }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.vinCode') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.vin_code }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.brand') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.comtran_brand }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.model') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.comtran_model }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.year') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.year }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.truckCategory') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.comtran_category }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.milage') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.odometer }} {{ t('publications.km') }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.engineType') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.engine_type }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.engineSm') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.engine }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.power') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.power }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.color') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.color }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.crash') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.crash ? t('publications.yes') : t('publications.no') }}
						</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.tradeIn') }}</div>
						<div class="font-semibold text-gray-800">{{ getTradeInLabel(truck.trade_in) }}
						</div>
					</div>
					<div>
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.owners') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.owners }}
						</div>
					</div>
					<div class="col-span-2">
						<div class="text-sm text-gray-500 mb-1">{{ t('publications.description') }}</div>
						<div class="font-semibold text-gray-800">{{ truck.description }}</div>
					</div>
				</div>
			</div>
		...
		<PopUpModerate v-if="showPopUpModerate" @save="moderateTruck" @cancel="closePopUpModerate"
...
<script setup lang="ts">
	...
	const truck = ref<ITruck | null>(null)
	const loading = ref(false)

	const breadcrumbs = computed(() => [
		{ label: t('sidebar.publications') },
		{ label: t('publications.trucks'), to: '/publications/trucks' },
		{ label: truck.value ? `${truck.value.comtran_brand} ${truck.value.comtran_model}` : t('publications.truckDetail.back') }
	])
	...
	async function getTruck() {
		try {
			...
			const data: ITruck = await api.getTruck({ id: truckId })
			truck.value = data
		} catch (error) {
			console.error('Ошибка при загрузке грузовика:', error)
		} finally {
			loading.value = false
		}
	}
	...
	function confirmDelete(event: any) {
		confirm.require({
			...
			message: t('publications.confirmDeleteTruck'),
	...
	async function deleteTruck() {
		...
		try {
			const truckId = route.params.id as string
			await api.deleteTruck({ id: truckId })
			toast.add({
				severity: 'success',
	...
	async function moderateTruck(formData: { status: number; title?: string; description?: string }) {
		if (!truck.value) return

		try {
			loadingPopUpModerate.value = true
			const truckId = route.params.id as string
			await api.moderateTruck({
				data: {
					id: Number(truckId),
			...
			console.error('Ошибка при модерации грузовика:', error)
			toast.add({
				severity: 'error',
				summary: t('base.error'),
				detail: t('toast.errorSaving'),
