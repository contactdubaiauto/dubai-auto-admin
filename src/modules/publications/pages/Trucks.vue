<template>
	<div class="h-full flex flex-col">
		...
		<div ref="trucksContainer" class="flex-1 overflow-y-auto" @scroll="handleScroll">
			<DataTable :value="trucks" :loading="loading" rowHover stripedRows size="small" @row-click="onRowClick">
				<Column field="id" header="ID" class="w-10"></Column>
				<Column :header="t('publications.truckDetail.info')" class="w-46">
				...
				<Column :header="t('publications.truckDetail.user')">
					<template #body="slotProps">
						<div class="flex items-center gap-2">
				...
				<Column :header="t('publications.truckDetail.status')">
					<template #body="slotProps">
						<span :class="[
...
<script setup lang="ts">
	...
	async function getTrucks() {
		...
		try {
			...
			const data: ITruck[] = await api.getTrucks({ params })

			if (data && data.length > 0) {
				if (lastId.value === null) {
					trucks.value = data
				} else {
					trucks.value = [...trucks.value, ...data]
				}
			...
			console.error('Error on get trucks:', error)
		} finally {
			loading.value = false
	...
	function handleScroll() {
		if (!trucksContainer.value || loading.value || loadingMore.value || !hasMore.value) {
			return
		}

		const container = trucksContainer.value
	...
	function onRowClick(event: any) {
		const truck = event.data as ITruck
		if (truck && truck.id) {
			router.push(`/publications/trucks/${truck.id}`)
		}
	}
