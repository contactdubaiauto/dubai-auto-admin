<template>
	<div class="h-full flex flex-col">
		...
		<div ref="carsContainer" class="flex-1 overflow-y-auto" @scroll="handleScroll">
			<DataTable :value="cars" :loading="loading" rowHover stripedRows size="small" @row-click="onRowClick">
				<Column field="id" header="ID" class="w-10"></Column>
				<Column :header="t('publications.carDetail.info')" class="w-46">
				...
				<Column :header="t('publications.carDetail.user')">
					<template #body="slotProps">
						<div class="flex items-center gap-2">
				...
				<Column :header="t('publications.carDetail.status')">
					<template #body="slotProps">
						<span :class="[
...
<script setup lang="ts">
	...
	const carsContainer = ref<HTMLElement | null>(null)
	...
	async function getCars() {
		...
		try {
			...
			const data: ICar[] = await api.getCars({ params })
	...
	function handleScroll() {
		if (!carsContainer.value || loading.value || loadingMore.value || !hasMore.value) {
			return
		}

		const container = carsContainer.value
	...
	function onRowClick(event: any) {
		const car = event.data as ICar
		if (car && car.id) {
			router.push(`/publications/cars/${car.id}`)
		}
	}
