<template>
				...
				<Column :header="t('settings.adminUser.permissions')">
					<template #body="slotProps">
						<div class="flex flex-wrap gap-1">
							<Tag v-for="(permission, idx) in slotProps.data.permissions" :key="idx"
								:value="getPermissionLabel(permission)" severity="info" class="text-xs" />
				...
				<Column :header="t('base.actions')" class="w-24">
					<template #body="slotProps">
						<div class="flex gap-1">
							<Button @click.stop="selectAdminUser(slotProps.data)" icon="pi pi-pencil" rounded variant="outlined"
								size="small" />
							<Button @click.stop="selectDeleteAdminUser(slotProps.data)" icon="pi pi-trash" severity="danger" rounded
	...
	<PopUpAdminUser v-if="showPopUpAdminUser" @save="saveAdminUser" @cancel="closePopUpAdminUser"
		:loading="loadingPopUpAdminUser" :item="selectedAdminUser" />
	<PopUpConfirmDelete v-if="showPopUpDeleteAdminUser" @delete="deleteAdminUser" @cancel="closePopUpDeleteAdminUser"
		:loading="loadingPopUpDeleteAdminUser" :description="t('settings.adminUser.confirmDelete')" />
	<Toast />
</template>
...
<script setup lang="ts">
	...
	async function getAdminUsers() {
		try {
			...
			const data: IAdminUser[] = await api.getAdminUsers({
				params: {
					role_id: 0
				}
			})
